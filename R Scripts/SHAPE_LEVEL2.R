# The R script for SHAPE: Spatial Health and Population Estimator - LEVEL TWO
...
# - this includes two predicted health risk behaviors from level 1!
selected_data <- select(data, ID, male, female, race1, race2, race3, race4, 
                        ...
                        insured, n_insured, urban, n_urban, smoker, n_smoker, obesity, n_obesity)
...
census <- read_excel("county_spatial_ipf_data_2023_with_smoking&obesity_PART1.xlsx")
...
census_select <- select(census, FIPS, male, female, race1, race2, race3, race4, age1, age2, age3, age4, n_bach, bach, income1, income2, income3, income4, 
                        insured, n_insured, urban, n_urban, smoker22, n_smoker22, obesity22, n_obesity22)
...
sum_diff_smoking <- sum((census$smoker22 + census$n_smoker22) - census$total_pop_18_o)

# Obesity
sum_diff_obesity <- sum((census$obesity22 + census$n_obesity22) - census$total_pop_18_o)
...
checks <- list(
  ...
  smoker = sum_diff_smoking, 
  obesity = sum_diff_obesity
)
...
weights8 <- array(dim = c(nrow(selected_data), nrow(census_select)))
weights9 <- array(dim = c(nrow(selected_data), nrow(census_select)))
...
cen.agg8 <- array(dim = c(nrow(census_select), ncol(census_select)-1))
cen.agg9 <- array(dim = c(nrow(census_select), ncol(census_select)-1))
...
for (i in 1:nrow(census_select2)) {
  cen.agg7[i, ] <- colSums(selected_data2 * weights0[, i] * weights1[, i] * weights2[,i]*weights3[,i]*weights4[,i]*weights5[,i]*weights6[,i]*weights7[,i])}
...
################ CONSTRAINT 8: SMOKING ###################

# Iterate through each row of spatial aggregate data 
# For each row, we are aiming to calculate ratios between aggregate and individual-level data for each smoking level
for (j in 1:nrow(census_select2)) {
  weights8[which(data$smoker == 1), j] <- census_select2[j, 21]/cen.agg7[j,21]
  weights8[which(data$n_smoker == 1), j] <- census_select2[j, 22]/cen.agg7[j,22]
}

for (i in 1:nrow(census_select2)) {
  cen.agg8[i, ] <- colSums(selected_data2 * weights0[, i] * weights1[, i] * weights2[,i]*weights3[,i]*weights4[,i]*weights5[,i]*weights6[,i]*weights7[,i]*weights8[,i])}


## --- Evaluating Results from eighth Iteration   --- ##
# creates a scatter plot comparing the model output 
plot(as.vector(as.matrix(census_select2)), as.vector(as.matrix(cen.agg8)), xlab = "Constraints", 
     ylab = "Model output")
abline(a = 0, b = 1)
...
################ CONSTRAINT 9: OBESITY ###################

# Iterate through each row of spatial aggregate data 
# For each row, we are aiming to calculate ratios between aggregate and individual-level data for each obesity level
for (j in 1:nrow(census_select2)) {
  weights9[which(data$obesity == 1), j] <- census_select2[j, 23]/cen.agg8[j,23]
  weights9[which(data$n_obesity == 1), j] <- census_select2[j, 24]/cen.agg8[j,24]
}

weights10 <- weights0 * weights1 * weights2 * weights3 * weights4 * weights5* weights6 * weights7 * weights8 * weights9

for (i in 1:nrow(census_select2)) {
  cen.agg9[i, ] <- colSums(selected_data2 * weights10[, i])}



## --- Evaluating Results from ninth Iteration   --- ##
# creates a scatter plot comparing the model output 
plot(as.vector(as.matrix(census_select2)), as.vector(as.matrix(cen.agg9)), xlab = "Constraints", 
     ylab = "Model output")
abline(a = 0, b = 1)

# Calculating correlation 
cor(as.vector(as.matrix(census_select2)), as.vector(as.matrix(cen.agg9)))
...
sum(weights10)-sum(census$total_pop_18_o)

# Save weights as a dataframe, then write to an excel file
df <- as.data.frame(weights10)
...
write_xlsx(df, "US_counties_part1_BRFSS22_ints_level2.xlsx")
...
## CHECK IF THE COUNT OF ROWS IS EQUAL TO SUM OF TOTAL POPULATION IN THE STUDY AREA!!!
...
## THIS IS THE END OF LEVEL TWO! YOU CAN JOIN THESE ESTIMATES TO OTHER DATASETS FOR EVALUATION PROCESSES!